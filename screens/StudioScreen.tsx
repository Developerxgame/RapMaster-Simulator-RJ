...
const StudioScreen: React.FC = () => {
    ...
    const TabButton: React.FC<{tab: StudioTab, label: string, icon: React.ReactNode}> = ({tab, label, icon}) => (
        <button onClick={() => setActiveTab(tab)} className={`flex-1 flex justify-center items-center space-x-2 py-2 text-sm font-semibold rounded-t-lg transition-colors ${activeTab === tab ? 'bg-ios-bg-secondary text-ios-blue' : 'bg-transparent text-ios-label-secondary'}`}>
            {icon}
            <span>{label}</span>
        </button>
    );
                    ...
                    <div className="bg-ios-bg-secondary p-4 rounded-xl space-y-4">
                        ...
                        <div>
                            ...
                            <input
                              ...
                              className="w-full bg-black text-ios-label p-2 rounded-lg border border-gray-700 focus:ring-2 focus:ring-ios-blue focus:outline-none"
                            />
                        </div>
                        ...
                        <button onClick={handleCreateTrack} disabled={!trackTitle.trim() || player.stats.energy < 20} className={`w-full py-3 bg-ios-blue text-white font-bold rounded-lg shadow-md disabled:bg-ios-gray ${animateRecord ? 'animate-pulse' : ''}`}>
                            Record Track
                        </button>
                    </div>
                    ...
                    <div className="bg-ios-bg-secondary p-4 rounded-xl">
                         ...
                         <div className="max-h-60 overflow-y-auto space-y-2">
                            {unreleasedTracks.length > 0 ? unreleasedTracks.slice().reverse().map(track => (
                                <div key={track.id} className="flex justify-between items-center bg-black p-2 rounded-md">
                                    <div>
                                        <p className="font-semibold">{track.title}</p>
                                        <p className="text-xs text-ios-label-secondary">Quality: {track.quality}</p>
                                    </div>
                                    <button onClick={() => handleReleaseTrack(track.id)} className="px-3 py-1 text-sm bg-ios-green text-black font-bold rounded-lg">Release as Single</button>
                                </div>
            ...
            {activeTab === 'album' && (
                <div className="space-y-4">
                    <div className="bg-ios-bg-secondary p-4 rounded-xl space-y-4">
                        ...
                        <input type="text" value={albumTitle} onChange={e => setAlbumTitle(e.target.value)} placeholder="Album Title" className="w-full bg-black text-ios-label p-2 rounded-lg border border-gray-700"/>
                        
                        <div className="max-h-60 overflow-y-auto space-y-2 pr-2">
                            <h3 className="text-lg font-semibold">Select Tracks ({selectedTrackIds.length}/10)</h3>
                            {availableForAlbum.map(track => (
                                <div key={track.id} onClick={() => handleToggleTrackForAlbum(track.id)} className={`flex justify-between items-center p-2 rounded-md cursor-pointer ${selectedTrackIds.includes(track.id) ? 'bg-ios-blue' : 'bg-black'}`}>
                                    <p>{track.title}</p>
                                    <p className="text-sm text-ios-label-secondary">Q: {track.quality}</p>
                                </div>
                            ))}
                        </div>
                        <button onClick={handleCreateAlbum} disabled={!albumTitle.trim() || player.stats.energy < 60 || selectedTrackIds.length < 3 || selectedTrackIds.length > 10} className="w-full py-3 bg-ios-green text-black font-bold rounded-lg shadow-md disabled:bg-ios-gray">
                            Release Album
                        </button>
                    </div>
                    <div className="bg-ios-bg-secondary p-4 rounded-xl">
                        <h2 className="text-xl font-semibold mb-2">Released Albums</h2>
                         <div className="max-h-60 overflow-y-auto space-y-2">
                            {discography.albums.length > 0 ? discography.albums.slice().reverse().map(album => (
                                <div key={album.id} className="bg-black p-2 rounded-md">
            ...
            {activeTab === 'mv' && (
                <div className="space-y-4">
                    <div className="bg-ios-bg-secondary p-4 rounded-xl space-y-4">
                        ...
                        <select value={selectedTrackForMV} onChange={e => setSelectedTrackForMV(e.target.value)} className="w-full bg-black text-ios-label p-2 rounded-lg border border-gray-700">
                            <option value="">Select a Released Track</option>
                            {releasedTracks.map(t => <option key={t.id} value={t.id}>{t.title}</option>)}
                        </select>
                        <button onClick={handleCreateMV} disabled={!selectedTrackForMV || player.stats.energy < 40} className="w-full py-3 bg-ios-red text-white font-bold rounded-lg shadow-md disabled:bg-ios-gray">
                            Shoot Video
                        </button>
                    </div>
                    <div className="bg-ios-bg-secondary p-4 rounded-xl">
                        <h2 className="text-xl font-semibold mb-2">My Music Videos</h2>
                         <div className="max-h-60 overflow-y-auto space-y-2">
                            {discography.musicVideos.length > 0 ? discography.musicVideos.slice().reverse().map(mv => (
                                <div key={mv.id} className="flex justify-between items-center bg-black p-2 rounded-md">
                                    <div>
